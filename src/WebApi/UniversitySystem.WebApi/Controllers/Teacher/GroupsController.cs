...
namespace UniversitySystem.WebApi.Controllers.Teacher
{
    ...
    public class GroupsController : ControllerBase
    {
        ...
        {
            GroupCreateCommand command = new GroupCreateCommand() { Name = dto.Name, CourseId = dto.CourseId, SpecializationId = dto.SpecializationId};
        ...
        {
            GroupUpdateCommand command = new GroupUpdateCommand(id) { Name = dto.Name, CourseId = dto.CourseId, SpecializationId = dto.SpecializationId};
            int value = await _mediator.Send(command);
            if(value == 0) return BadRequest();
            if (value == -1) return BadRequest(new { code = "relation", description = "no such relationship exists" });
            return StatusCode(StatusCodes.Status201Created, value);
